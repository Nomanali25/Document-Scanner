...
class DocScanner:
    """An image scanner for document detection and transformation."""
    ...
    def angle_range(self, quad):
        """Calculate the range of interior angles of a quadrilateral."""
        tl, tr, br, bl = quad
        ura = self.get_angle(tl[0], tr[0], br[0])
        ula = self.get_angle(bl[0], tl[0], tr[0])
        lra = self.get_angle(tr[0], br[0], bl[0])
    ...
    def get_corners(self, img):
        """Detect corners in the image using OpenCV's line detection."""
    ...
    def is_valid_contour(self, cnt, IM_WIDTH, IM_HEIGHT):
        """Check if a contour is valid based on area and angle range."""
        return (len(cnt) == 4 and cv2.contourArea(cnt) > IM_WIDTH * IM_HEIGHT * self.MIN_QUAD_AREA_RATIO
                and self.angle_range(cnt) < self.MAX_QUAD_ANGLE_RANGE)
    ...
    def get_contour(self, rescaled_image):
        ...
        if len(test_corners) >= 4:
            ...
            # Sort quadrilaterals by area and angle range
            quads = sorted(quads, key=cv2.contourArea, reverse=True)[:5]
            quads = sorted(quads, key=self.angle_range)
        ...
        # Also try finding contours directly
        (cnts, hierarchy) = cv2.findContours(edged.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        cnts = sorted(cnts, key=cv2.contourArea, reverse=True)[:5]
        ...
        # If no valid contours, use the whole image
        if not approx_contours:
            TOP_RIGHT = (IM_WIDTH, 0)
            BOTTOM_RIGHT = (IM_WIDTH, IM_HEIGHT)
            BOTTOM_LEFT = (0, IM_HEIGHT)
    ...
    def interactive_get_contour(self, screenCnt, rescaled_image):
        """Allow user to interactively adjust the document contour."""
        poly = Polygon(screenCnt, animated=True, fill=False, color="yellow", linewidth=5)
        fig, ax = plt.subplots()
        ax.add_patch(poly)
        ax.set_title('Drag the corners of the box to the corners of the document.\nClose the window when finished.')
    ...
    def scan(self, image_path):
        """Scan an image and save the transformed document."""
        RESCALED_HEIGHT = 500.0
        OUTPUT_DIR = 'output'
        ...
        print(f"Processed {basename} and saved to {output_path}")

        # Display the processed image
        cv2.imshow("Scanned Image", thresh)
        cv2.waitKey(0)
...
if __name__ == "__main__":
    ...
    # Scan all images in directory
        ...
        im_files = [f for f in os.listdir(im_dir) if get_ext(f) in valid_formats]
        for im in tqdm(im_files, desc="Processing images"):
